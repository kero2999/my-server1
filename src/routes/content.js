...
function renderHowToMakeSection(courseSlug, chapterNumber) {
  ...
  if (!howToMake) return "";
  const icons = ["fa-bullseye", "fa-user", "fa-lightbulb", "fa-chart-line", "fa-list-check", "fa-flag-checkered"];
  const steps = howToMake.steps.map((step) => `<article class="ql-how-slide" data-how-slide="${step.number - 1}"><div class="ql-how-number"><i class="fas ${icons[(step.number - 1) % icons.length]}" aria-hidden="true"></i><small>${String(step.number).padStart(2, "0")}</small></div><div><h3>${escapeHtml(step.title)}</h3><p><strong>ماذا نفعل؟</strong><br>${escapeHtml(step.what)}</p><p class="ql-how-example"><strong>مثال عملي:</strong><br>${escapeHtml(step.example)}</p><p><strong>لماذا فعلنا ذلك؟</strong><br>${escapeHtml(step.why)}</p></div></article>`).join("");
  const total = howToMake.steps.length;
  const script = `<script>(function(){var root=document.getElementById("ql-how-to-make");if(!root)return;var slides=[].slice.call(root.querySelectorAll("[data-how-slide]")),count=slides.length,index=0,indicator=root.querySelector("[data-how-indicator]"),show=function(next){index=(next+count)%count;slides.forEach(function(slide,i){slide.hidden=i!==index;slide.classList.toggle("is-active",i===index);});if(indicator)indicator.textContent=String(index+1).padStart(2,"0")+" / "+String(count).padStart(2,"0");};root.querySelector("[data-how-prev]").addEventListener("click",function(){show(index-1);});root.querySelector("[data-how-next]").addEventListener("click",function(){show(index+1);});show(0);})();</script>`;
  return `<style id="ql-how-to-make-style">.ql-how-to-make{width:min(1120px,calc(100% - 32px));margin:42px auto 120px;padding:30px;border:1px solid rgba(201,168,106,.42);border-radius:24px;background:linear-gradient(135deg,#332116 0%,#0f0d0b 100%);color:inherit;box-shadow:0 20px 48px rgba(0,0,0,.2);font-family:inherit}.ql-how-to-make h2{margin:0 0 7px;color:#f0d79a;font-size:clamp(1.25rem,2.4vw,1.9rem)}.ql-how-subtitle{margin:0 0 8px;font-size:1.05rem;font-weight:800}.ql-how-intro,.ql-how-result{line-height:1.9;color:rgba(255,255,255,.8)}.ql-how-slider{margin-top:22px;min-height:245px}.ql-how-slide{display:grid;grid-template-columns:58px 1fr;gap:16px;padding:22px;border:1px solid rgba(201,168,106,.3);border-radius:18px;background:linear-gradient(135deg,rgba(201,168,106,.16),rgba(10,10,12,.36));animation:qlHowIn .35s ease both}.ql-how-slide[hidden]{display:none}.ql-how-number{display:grid;place-items:center;align-content:center;gap:3px;width:48px;height:58px;border-radius:15px;background:linear-gradient(135deg,#c9a86a,#7a5230);color:#16100c;font-weight:900;font-size:1.05rem}.ql-how-number i{font-size:1.1rem}.ql-how-number small{font-size:.72rem}.ql-how-slide h3{margin:0 0 12px;color:#f0d79a;font-size:1.2rem}.ql-how-slide p{margin:7px 0;line-height:1.85;color:rgba(255,255,255,.82)}.ql-how-example{padding:10px 12px;border-inline-start:3px solid #c9a86a;background:rgba(201,168,106,.09)}.ql-how-controls{display:flex;align-items:center;justify-content:center;gap:14px;margin-top:16px}.ql-how-controls button{border:1px solid #c9a86a;background:rgba(201,168,106,.1);color:#f0d79a;border-radius:999px;padding:9px 16px;font:700 13px Tajawal,sans-serif;cursor:pointer;transition:transform .2s,background .2s}.ql-how-controls button:hover{transform:translateY(-2px);background:#c9a86a;color:#17110c}.ql-how-indicator{min-width:60px;text-align:center;color:#c9a86a;font-weight:900}.ql-how-result{margin:18px 0 0;padding:14px;border-radius:14px;background:rgba(63,143,125,.1);border:1px solid rgba(63,143,125,.3)}@keyframes qlHowIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}@media(max-width:640px){.ql-how-to-make{width:calc(100% - 20px);padding:18px;border-radius:18px}.ql-how-slide{grid-template-columns:40px 1fr;padding:15px;gap:10px}.ql-how-number{width:36px;height:52px;border-radius:11px;font-size:.82rem}.ql-how-slider{min-height:320px}}</style><section id="ql-how-to-make" class="ql-how-to-make" dir="rtl"><h2>🛠️ ${escapeHtml(howToMake.title)}</h2><p class="ql-how-subtitle">${escapeHtml(howToMake.subtitle)}</p><p class="ql-how-intro">${escapeHtml(howToMake.introduction)}</p><div class="ql-how-slider">${steps}</div><div class="ql-how-controls"><button type="button" data-how-prev>← السابق</button><span class="ql-how-indicator" data-how-indicator>01 / ${String(total).padStart(2, "0")}</span><button type="button" data-how-next>التالي →</button></div><p class="ql-how-result"><strong>النتيجة:</strong> ${escapeHtml(howToMake.result)}</p></section>${script}`;
}
