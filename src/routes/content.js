...
function renderHowToMakeSection(courseSlug, chapterNumber) {
  ...
  if (!howToMake) return "";
  const steps = howToMake.steps.map((step) => `<div class="ql-how-flow-card"><div class="ql-how-flow-number">${String(step.number).padStart(2, "0")}</div><h4>${escapeHtml(step.title)}</h4><p>${escapeHtml(step.example)}</p></div>`).join("");
  return `<style id="ql-how-to-make-style">.ql-how-to-make{width:min(1120px,calc(100% - 32px));margin:42px auto 120px;padding:30px;border:1px solid rgba(201,168,106,.42);border-radius:24px;background:linear-gradient(135deg,#452b1c 0%,#17100c 100%);color:inherit;box-shadow:0 20px 48px rgba(0,0,0,.22);font-family:inherit}.ql-how-header{display:flex;align-items:center;gap:14px;margin-bottom:16px}.ql-how-icon{display:grid;place-items:center;width:54px;height:54px;border-radius:16px;background:linear-gradient(135deg,#c9a86a,#7a5230);color:#17100c;font-size:1.35rem}.ql-how-to-make h2{margin:0;color:#f0d79a;font-size:clamp(1.25rem,2.4vw,1.9rem)}.ql-how-subtitle{margin:3px 0 0;font-size:1.05rem;font-weight:800}.ql-how-intro,.ql-how-result{line-height:1.9;color:rgba(255,255,255,.84)}.ql-how-case{margin:18px 0;padding:18px;border-radius:18px;background:linear-gradient(135deg,rgba(124,77,255,.2),rgba(201,168,106,.1));border:1px solid rgba(201,168,106,.28)}.ql-how-case h3{margin:0 0 7px;color:#fff;font-size:1.15rem}.ql-how-case p{margin:5px 0;line-height:1.85;color:rgba(255,255,255,.8)}.ql-how-flow{display:grid;grid-template-columns:repeat(auto-fit,minmax(145px,1fr));gap:10px;margin:20px 0}.ql-how-flow-card{position:relative;padding:15px 12px;border:1px solid rgba(201,168,106,.28);border-radius:15px;background:rgba(10,10,12,.32);text-align:center;animation:qlHowIn .4s ease both}.ql-how-flow-card:not(:last-child)::after{content:'←';position:absolute;left:-11px;top:42%;color:#c9a86a;font-size:1.25rem}.ql-how-flow-number{margin:auto;width:34px;height:34px;display:grid;place-items:center;border-radius:50%;background:#c9a86a;color:#17100c;font-weight:900}.ql-how-flow-card h4{margin:9px 0 6px;color:#f0d79a;font-size:.98rem}.ql-how-flow-card p{margin:0;color:rgba(255,255,255,.78);line-height:1.65;font-size:.88rem}.ql-how-explain{padding:15px;border-inline-start:4px solid #7c4dff;background:rgba(124,77,255,.1);line-height:1.9;color:rgba(255,255,255,.86)}.ql-how-result{margin:16px 0 0;padding:15px;border-radius:14px;background:rgba(63,143,125,.13);border:1px solid rgba(63,143,125,.34)}@keyframes qlHowIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}@media(max-width:640px){.ql-how-to-make{width:calc(100% - 20px);padding:18px;border-radius:18px}.ql-how-flow{grid-template-columns:1fr 1fr}.ql-how-flow-card:not(:last-child)::after{display:none}}</style><section id="ql-how-to-make" class="ql-how-to-make" dir="rtl"><div class="ql-how-header"><div class="ql-how-icon"><i class="fas fa-person-chalkboard"></i></div><div><h2>🛠️ ${escapeHtml(howToMake.title)}</h2><p class="ql-how-subtitle">${escapeHtml(howToMake.subtitle)}</p></div></div><p class="ql-how-intro">${escapeHtml(howToMake.introduction)}</p><div class="ql-how-case"><h3><i class="fas fa-store"></i> المثال الواقعي: ${escapeHtml(howToMake.caseTitle)}</h3><p>${escapeHtml(howToMake.example)}</p><p>${escapeHtml(howToMake.caseDescription)}</p></div><div class="ql-how-flow">${steps}</div><div class="ql-how-explain"><strong>طريقة التفكير:</strong> ${escapeHtml(howToMake.explanation)}</div><p class="ql-how-result"><strong>النتيجة التي نصل إليها:</strong> ${escapeHtml(howToMake.result)}</p></section>`;
}
