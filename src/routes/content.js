...
function renderHowToMakeSection(courseSlug, chapterNumber) {
  ...
  const stepSlides = howToMake.steps.map((step) => `<article class="ql-how-slide" data-slide-index="${step.number}"><div class="ql-how-slide-top"><span class="ql-how-step-no">STEP ${String(step.number).padStart(2, "0")}</span><span class="ql-how-visual-label"><i class="fas ${iconFor(step.visualType)}"></i> مثال تطبيقي</span></div><h3>${escapeHtml(step.title)}</h3><p class="ql-how-explain"><strong>ماذا فعل المحترف؟</strong> ${escapeHtml(step.what)}</p><div class="ql-how-example-card"><div class="ql-how-card-label">EXAMPLE — المثال المكتمل</div><div class="ql-how-card-value">${escapeHtml(step.filledValue)}</div></div><p class="ql-how-why"><strong>كيف وصلنا للقرار؟</strong> ${escapeHtml(step.explanation)}</p><p class="ql-how-why"><strong>لماذا هذه الخطوة؟</strong> ${escapeHtml(step.why)}</p></article>`).join("");
