...
router.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    if (process.env.BREVO_API_KEY && process.env.EMAIL_FROM) {
      ...
      sendSmtpEmail.subject = "إعادة تعيين كلمة السر — منصة التسويق التعليمية";
...
<p>
إذا لم تطلب إعادة تعيين كلمة السر فتجاهل هذه الرسالة.
</p>
...
`;
      ...
      console.warn("⚠️ BREVO_API_KEY أو EMAIL_FROM غير مضبوطين — لم يتم إرسال إيميل. رابط إعادة التعيين (للتجربة فقط):", resetUrl);
    }
...
router.post("/reset-password", async (req, res) => {
  try {
    ...
    console.error(e);
    return res.status(500).json({ ok: false, error: "حصل خطأ غير متوقع، حاول مرة أخرى." });
  }
});
