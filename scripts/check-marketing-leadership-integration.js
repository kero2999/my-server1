...
for (let chapter = 1; chapter <= 12; chapter += 1) {
  ...
  if (!getMentorProjectPrompt("marketing-leadership", chapter)) {
    throw new Error(`Mentor project prompt is missing for chapter ${chapter}`);
  }
}

const project = getGraduationProjectBrief({ slug: "marketing-leadership" });
if (!project || project.deliverables.length < 8) {
  throw new Error("Marketing Leadership graduation project is incomplete");
}
...
if (!howToMake.includes('id="ql-how-to-make"') || !howToMake.includes("المشروع المقترح") || !howToMake.includes("هدف المشروع") || !howToMake.includes("كيف نفذنا المشروع خطوة بخطوة") || !howToMake.includes("مثال مكتمل أمامك") || !howToMake.includes("كيف وصلنا للقرار؟")) {
